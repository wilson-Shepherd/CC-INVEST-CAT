...
import { createGrid, executeTrade, getAccountBalance, getBTCUSDPrice } from '../../utils/tradingUtils.js';
...
function openWebSocket() {
  const ws = new WebSocket(`wss://testnet.binance.vision/ws/${symbol.toLowerCase()}@kline_${interval}`);
...
async function handleNewPrice(price, closeTime) {
  ...
  try {
    ...
    if (totalAssetsInUSD < 50) {  // 假設最小交易單位需要至少 50 美元
    ...
    for (let i = 0; i < grid.length - 1; i++) {
      ...
      if (price <= gridLower && USDT >= price * (1 / gridSize) && (!lastTradePrices.has(gridLower) || currentTime - lastTradePrices.get(gridLower) > tradeCooldown)) {
      ...
      } else if (price >= gridUpper && BTC >= (1 / gridSize) && (!lastTradePrices.has(gridUpper) || currentTime - lastTradePrices.get(gridUpper) > tradeCooldown)) {
        const quantity = (1 / gridSize).toFixed(8);
