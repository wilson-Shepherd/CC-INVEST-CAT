export function gridTrading(
  ...
  historicalData.forEach((data) => {
    ...
    console.log(
      `Processing data point - Time: ${new Date(time).toISOString()}, Price: ${price}`,
    );
    ...
    for (let i = 0; i < grid.length - 1; i++) {
      ...
      if (price <= gridUpper && price > gridLower) {
        ...
        if (quantity > 0) {
          ...
          console.log(
            `Buy executed - Price: ${price}, Quantity: ${quantity}, New Balance: ${balance}, New Position: ${position}`,
          );
          actionTaken = true;
        } else {
        ...
        if (quantity > 0) {
          ...
          console.log(
            `Sell executed - Price: ${price}, Quantity: ${quantity}, New Balance: ${balance}, New Position: ${position}, Profit/Loss: ${profitLoss}, Total P/L: ${totalProfitLoss}`,
          );
          actionTaken = true;
        } else {
